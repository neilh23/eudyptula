...
public:
  ...
  */
  ...
  void setAmplitude( StkFloat amplitude );
...
{
  if ( table_.empty() ) {
    std::cerr << "Tick: Table was empty, resizing ..." << std::endl;
...
{
  if (table_.empty()) {
    //std::cerr << "SineWave table was empty ..." << std::endl;
    return frames;
  }
